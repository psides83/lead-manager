import moment from "moment";
import {
  doc,
  setDoc,
} from "firebase/firestore";
import { db } from "./firebase";

// Sends email when equipment is updated:
const sendQuoteLinkOpenedEmail = async (lead) => {
  // Creates the paramaters for the email template
  const timestamp = moment().format("DD-MMM-yyyy hh:mmA");
  const emailID = moment().format("yyyyMMDDHHmmss");
  const recipient = "psides@sunsouth.com";
  const subject = `Quote link opened by ${lead.name}`;
  const body = `<body style="font-family: Helvetica;">
                    ${tractorSVGLogo}
                    <section>
                        <p>${timestamp}</p>
                        <p><strong>${lead.name}</strong> opened the link to their quote from the Lead Manager app</p>
                    </section>
                    <body>`;

  // Sets paramaters for the email template
  const emailData = {
    to: recipient,
    replyTo: recipient,
    from: "Lead Manager App<psides.solutions@outlook.com>",
    message: {
      subject: subject,
      html: body,
    },
  };

  // Sends the email
  await setDoc(doc(db, "sentEmails", emailID), emailData);
  // console.log(recipients)
};

const tractorSVGLogo = `<div style="display: flex; flex-direction: row; font-family: Helvetica; align-items: flex-end; font-size: 68px;">
                          <div style=" width: 88px; height: 88px; background-color: #367C2B; border-radius: 50px; margin-right: 10px; ">
                            <svg style=" margin: 10px; fill: white; " focusable="false" aria-hidden="true" viewBox="0 0 24 24" data-testid="AgricultureRoundedIcon">
                              <path d="M19.5 11.97c.93 0 1.78.28 2.5.76V7.97c0-1.1-.9-2-2-2h-6.29l-1.06-1.06 1.06-1.06c.2-.2.2-.51 0-.71s-.51-.2-.71 0l-2.83 2.83c-.2.2-.2.51 0 .71.2.2.51.2.71 0l1.06-1.06L13 6.68v2.29c0 1.1-.9 2-2 2h-.54c.95 1.06 1.54 2.46 1.54 4 0 .34-.04.67-.09 1h3.14c.25-2.24 2.14-4 4.45-4z"></path>
                              <path d="M19.5 12.97c-1.93 0-3.5 1.57-3.5 3.5s1.57 3.5 3.5 3.5 3.5-1.57 3.5-3.5-1.57-3.5-3.5-3.5zm0 5c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zM4 8.97h5c0-1.1-.9-2-2-2H4c-.55 0-1 .45-1 1 0 .56.45 1 1 1zm5.83 4.82-.18-.47.93-.35c-.46-1.06-1.28-1.91-2.31-2.43l-.4.89-.46-.21.4-.9c-.55-.21-1.17-.35-1.81-.35-.53 0-1.04.11-1.52.26l.34.91-.47.18L4 10.4c-1.06.46-1.91 1.28-2.43 2.31l.89.4-.21.46-.9-.4c-.22.55-.35 1.16-.35 1.8 0 .53.11 1.04.26 1.52l.91-.34.18.47-.93.35c.46 1.06 1.28 1.91 2.31 2.43l.4-.89.46.21-.4.9c.57.22 1.18.35 1.82.35.53 0 1.04-.11 1.52-.26l-.35-.91.47-.18.35.93c1.06-.46 1.91-1.28 2.43-2.31l-.89-.4.21-.46.9.4c.22-.57.35-1.18.35-1.82 0-.53-.11-1.04-.26-1.52l-.91.35zm-2.68 3.96c-1.53.63-3.29-.09-3.92-1.62-.63-1.53.09-3.29 1.62-3.92 1.53-.63 3.29.09 3.92 1.62.64 1.53-.09 3.28-1.62 3.92z"></path>
                            </svg>
                          </div> 
                          <div style="color: #367C2B; font-size: 68px; ">
                            <strong>
                              Lead Manager
                            </strong>
                          </div>
                        </div>`

export { sendQuoteLinkOpenedEmail };
